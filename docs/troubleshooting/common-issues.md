...
# Check events
kubectl get events -n dclaw-migrate --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-migrate deployment/dclaw-migrate-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-migrate-db-credentials -n dclaw-migrate
   ```
